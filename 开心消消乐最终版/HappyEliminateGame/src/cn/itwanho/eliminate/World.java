        ...
        for (int row = ROWS - 1; row >= 0; row--) {//倒着遍历行
            for (int col = COLS - 1; col >= 0; col--) {//倒着遍历列
                if (elements[row][col] == null){//元素为null,则跳过本次循环
                    ...
                    //1)查找一行中连续的个数
                    int rowNum = 0;//该行与当前元素相邻且相同元素的个数
                    ...
                    if (rowNum >= 2){//当前行有三个以上相同并且相邻的元素
                        number= number+rowNum+1;
                        elements[row][col].setEliminated(true);//将当前元素设置为可消状态
                        for (int i = 0; i < rowNum; i++) {//将当前行与当前元素相同且相邻的元素设置为可消状态
                            elements[row][col - 1 - i].setEliminated(true);
                        }
                    }
                    if (colNum >= 2){//当前列有三个以上相同并且相邻的元素
                        number= number+colNum+1;
                        elements[row][col].setEliminated(true);//将当前元素设置为可消状态
                        for (int i = 0; i < colNum; i++) {//将当前列与当前元素相同且相邻的元素设置为可消状态
                    ...
                    if (rowNum >= 2){//当前行有三个以上相同并且相邻的元素
                        ...
                        for (int i = 0; i < rowNum; i++) {//将当前行与当前元素相同且相邻的元素设置为null,即被消除
                    ...
                    if (colNum >= 2){//当前列有三个以上相同并且相邻的元素
                        elements[row][col] = null;//将当前元素设置为null,即被消除
                        for (int i = 0; i < colNum; i++) {//将当前列与当前元素相同且相邻的元素设置为null,即被消除
                        ...
                        new Thread(){
                            ...
                            public void run() {//自动执行
                                ...
                                if (deleteElements()){//可消
                                    ...
                                    }while (deleteElements());//是否可消
                                }else {//不可消
                                    moveElement();//移动两个元素（动画移动）复原
                                    exchangeElement();//交换两个元素复原
                                }
                                deleteElements();//消除
                                canInteractive=true;//可交互
                            }
